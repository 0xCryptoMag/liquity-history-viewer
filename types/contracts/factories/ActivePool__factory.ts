...
import type { ActivePool, ActivePoolInterface } from '../ActivePool.js';
...
export class ActivePool__factory {
	...
	static createInterface<M extends Modifiers>(
		modifiers: M
	): ActivePoolInterface<M> {
		if (modifiers.abisType === 'normal') return new Interface(_abi) as ActivePoolInterface<M>;

		return new Interface(
			modifyAbi(structuredClone(_abi), modifiers)
		) as ActivePoolInterface<M>;
	}
