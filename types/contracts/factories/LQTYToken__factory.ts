...
import type { LQTYToken, LQTYTokenInterface } from '../LQTYToken.js';
...
export class LQTYToken__factory {
	...
	static createInterface<M extends Modifiers>(
		modifiers: M
	): LQTYTokenInterface<M> {
		if (modifiers.abisType === 'normal') return new Interface(_abi) as LQTYTokenInterface<M>;

		return new Interface(
			modifyAbi(structuredClone(_abi), modifiers)
		) as LQTYTokenInterface<M>;
	}
