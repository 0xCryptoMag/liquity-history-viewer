...
import type { BorrowerOperations, BorrowerOperationsInterface } from '../BorrowerOperations.js';
...
export class BorrowerOperations__factory {
	...
	static createInterface<M extends Modifiers>(
		modifiers: M
	): BorrowerOperationsInterface<M> {
		if (modifiers.abisType === 'normal') return new Interface(_abi) as BorrowerOperationsInterface<M>
		return new Interface(
			modifyAbi(structuredClone(_abi), modifiers)
		) as BorrowerOperationsInterface<M>;
	}
	static connect<M extends Modifiers>(
		address: string,
		modifiers: M,
		runner?: ContractRunner | null
	): BorrowerOperations<M> {
		if (modifiers.abisType === 'normal')
			return new Contract(
				address,
				_abi,
				runner
			) as unknown as BorrowerOperations<M>
