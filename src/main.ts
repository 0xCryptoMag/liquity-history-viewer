...
document.addEventListener('DOMContentLoaded', () => {
	...
	const stabilityPoolTableReset = document.querySelector(
		'.stability-pool-updates'
	).innerHTML;
...
function creatSubmitHandler(troveTableReset, stabilityPoolTableReset) {
	...
	button.addEventListener('click', async event => {
		...
		const stabilityPoolTable = document.getElementById(
			'stability-pool-updates'
		);
...
function displayTroveData(queryData, protocol) {
	...
	troveData.forEach((log, ind, arr) => {
		...
		if (log[7] === 'redeemCollateral' && log[5] === 0n) {
			/** @type {number} */
			const collateralSurplusIndex = collateralSurplus.findIndex(
				element => log[1] === element[1]
			);
		...
		collAfter.textContent = formatDecimals(
			log[5],
			decimals[protocol],
			false
		);
		...
		collChange.textContent = formatDecimals(
			collChangeAmount,
			decimals[protocol],
			false
		);
		...
		debtAfter.textContent = formatDecimals(
			log[4],
			decimals[protocol],
			false
		);
		...
		debtChange.textContent = formatDecimals(
			debtChangeAmount,
			decimals[protocol],
			false
		);
...
function displayStabilityPoolData(queryData, protocol) {
	...
	liquidations.forEach(liq => {
		// User deposits
		let userDepositsDuringLiquidationIndex =
			userDepositUpdates.findIndex(element => element[1] > liq[1]) - 1;

		if (
			userDepositUpdates?.[userDepositsDuringLiquidationIndex]?.[1] ===
				liq[1] &&
			userDepositUpdates?.[userDepositsDuringLiquidationIndex]?.[2] >
				liq[2]
		) {
			userDepositsDuringLiquidationIndex -= 1;
		}

		const userDepositsDuringLiquidation =
			userDepositUpdates?.[userDepositsDuringLiquidationIndex]?.[4];
		const userDepositDuringLiquidationBlock =
			userDepositUpdates?.[userDepositsDuringLiquidationIndex]?.[1];

		if (!userDepositsDuringLiquidation) return;

		// Total deposits
		const totalDepositsDuringLiquidationIndex =
			totalDeposits.findIndex(
				element => element[1] === liq[1] && element[2] === liq[2]
			) - 1;
		const totalDepositsDuringLiquidation =
			totalDeposits?.[totalDepositsDuringLiquidationIndex]?.[3];
		const totalDepositsDuringLiquidationBlock =
			totalDeposits?.[totalDepositsDuringLiquidationIndex]?.[1];
		...
		const poolOnwershipAmount =
			(userDepositsDuringLiquidation *
				10n ** BigInt(decimals[protocol])) /
			totalDepositsDuringLiquidation;
		...
		stakeBalanceAfter.textContent = formatDecimals(
			totalDepositsDuringLiquidation * 10n ** BigInt(decimals[protocol]) -
				stakeReductionAmount,
			2 * decimals[protocol],
			true
		);
		...
		stakeReduction.textContent = formatDecimals(
			stakeReductionAmount,
			2 * decimals[protocol],
			true
		);
		...
		collGain.textContent = formatDecimals(
			collGainAmount,
			2 * decimals[protocol],
			true
		);
		...
		poolOwnership.textContent = formatDecimals(
			poolOnwershipAmount,
			decimals[protocol]
		);
...
function formatDecimals(val, dec, scale) {
	...
	if (dec < 0) {
		...
		return (
			stringifiedVal.slice(0, stringifiedVal.length - dec) +
			'.' +
			stringifiedVal
				.slice(stringifiedVal.length - dec)
				.slice(0, scale ? dec / 2 + 1 : dec + 1)
		);
	} else {
		return (
			'0.' +
			('0'.repeat(dec - stringifiedVal.length) + stringifiedVal).slice(
				0,
				scale ? dec / 2 + 1 : dec + 1
			)
		);
